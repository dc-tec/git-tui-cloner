...
impl GitServiceTrait for GitHubService {
    async fn search_repositories(&self, query: &String, page: u32) -> AppResult<Vec<Repository>> {
        if self.token.is_none() {
            println!("Warning: To prevent hitting rate limits, please provide a GitHub token");
        };
        ...
        .json()
        .await
        .map_err(|e| AppError::NetworkError(format!("Failed to parse GitHub API response: {}", e)))?;

        Ok(search_response.items.into_iter().map(|repo| Repository {
        ...
        async fn get_user_repositories(&self, page: u32) -> AppResult<Vec<Repository>> {
            if self.token.is_none() {
                return Err(AppError::AuthenticationError(
                    "GitHub token is required to retrieve user repositories".to_string()
                ));
            }

            let url = format!("https://api.github.com/user/repos?page={}&per_page=30", page);
            let response = self.make_request(&url).await?;
